...
MERGE (person:Person {
    ...
    ON CREATE SET person.personID = apoc.create.uuid();
MERGE (person:Person {
    given: "Scott",
    surname: "Wing",
    email: "wing@si.edu",
...
MERGE (reference1:Reference {
      ...
      })-[entered_by1:ENTERED_BY]->(person)
      ON CREATE SET
        reference1.referenceID = apoc.create.uuid(),
        entered_by1.timestamp = datetime()
MERGE (reference2:Reference {
      title: "Manual of Leaf Architecture",
      year: "2009",
      publisher: "Cornell University Press"
      })-[entered_by2:ENTERED_BY]->(person)
      ON CREATE SET
        reference2.referenceID = apoc.create.uuid(),
        entered_by2.timestamp = datetime();
...
MERGE (reference)-[:AUTHORED_BY {order: author.order}]->(person);
...
MATCH (person:Person {given: "Andrew", surname: "Zaffos"})
MERGE (schema1:Schema {
      title: "Smithsonian, 1999",
      year: "1999"
      })-[entered_by1:ENTERED_BY]->(person)
      ON CREATE SET
        schema1.schemaID = apoc.create.uuid(),
        entered_by1.timestamp = datetime()
MERGE (schema2:Schema {
      title: "Cornell, 2009",
      year: "2009"
      })-[entered_by2:ENTERED_BY]->(person)
      ON CREATE SET
        schema2.schemaID = apoc.create.uuid(),
        entered_by2.timestamp = datetime();

MATCH (schema:Schema {title: "Smithsonian, 1999"}), (reference:Reference {publisher: "Smithsonian Institution"})
MERGE (reference)-[:CITED_BY]->(schema);
...
MERGE (schema)-[:AUTHORED_BY {order: author.order}]->(person);
...
MERGE
    (character:Character {name: "Leaf Attachment"})-[:CHARACTER_OF]->(schema),
	(character)-[character_entered_by:ENTERED_BY]->(person),
		(state1:State {name: "alternate", definition: "one leaf at each node"})-[:STATE_OF]->(character),
		(state1)-[state1_entered_by:ENTERED_BY]->(person),
		(state2:State {name: "opposite", definition: "two leaves at each node"})-[:STATE_OF]->(character),
		(state2)-[state2_entered_by:ENTERED_BY]->(person),
		(state3:State {name: "whorled", definition: "three or more leaves at each node"})-[:STATE_OF]->(character),
		(state3)-[state3_entered_by:ENTERED_BY]->(person),
		(state4:State {name: "decussate", definition: "each leaf attached at 90 from those above and below (can be opposite or alternate)"})-[:STATE_OF]->(character),
...
MATCH (person:Person {given: "Rebecca", surname: "Koll"}), (schema:Schema {title: "Smithsonian, 1999"})
MERGE
    (character:Character {name: "Petiol Features"})-[:CHARACTER_OF]->(schema),
    (character)-[character_entered_by:ENTERED_BY]->(person),
		(state1:State {name: "base swollen", definition: "petiole thickens at the base where it attaches to the node"})-[:STATE_OF]->(character),
		(state1)-[state1_entered_by:ENTERED_BY]->(person),
		(state2:State {name: "pulvinate", definition: "having an abruptly swollen portion near the node around which the leaf can flex"})-[:STATE_OF]->(character),
		(state2)-[state2_entered_by:ENTERED_BY]->(person)
        ON CREATE SET
            character.characterID = apoc.create.uuid(),
            character_entered_by.timestamp = datetime(),
            state1.characterID = apoc.create.uuid(),
            state1_entered_by.timestamp = datetime(),
            state2.characterID = apoc.create.uuid(),
            state2_entered_by.timestamp = datetime();
