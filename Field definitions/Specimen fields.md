...
* dwc:  
* Info: The existing PBDB collection, or newly defined collection, that the specimen is associated with. PBot follows the definition for collection as set forth in the PBDB: “In the context of our database, a ‘fossil collection’ represents a set of fossil occurrences co-located geographically and temporally. In order to belong to a single collection the occurrences must have been collected deliberately as a group, though not necessarily all on the same occasion.”
* Entry type: select PBDB collection, or prompt to create collection record. **Required.**   
* Consideration: Does this fetch/search/display collection names from PBDB? Do all collections need to exist in PBot? Are we only entering collections when they don't exist in PBDB?
